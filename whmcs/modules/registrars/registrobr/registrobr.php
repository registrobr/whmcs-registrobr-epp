...
function registrobr_AdminCustomButtonArray() {
    return array(
        'Sincronizar DNS Padrao' => 'SyncDefaultDNS',
    );
}

function registrobr_SyncDefaultDNS($params) {

    require_once('RegistroEPP/RegistroEPPFactory.class.php');

    # Busca nameservers padrao do WHMCS
    $defaultNS = array();
    for ($i = 1; $i <= 5; $i++) {
        $result = localAPI('GetConfigurationValue', array('setting' => 'DefaultNameserver' . $i));
        if (!empty($result['value'])) {
            $defaultNS['ns' . $i] = $result['value'];
        } else {
            $defaultNS['ns' . $i] = '';
        }
    }
    ...
    if ($countNS < 2) {
        return array('error' => 'Nameservers padrao do WHMCS nao configurados. Configure em Setup > General Settings > Domains.');
    }

    # Monta params para SaveNameservers
    $setparams = $params;
    $setparams['ns1'] = $defaultNS['ns1'];
    $setparams['ns2'] = $defaultNS['ns2'];
    $setparams['ns3'] = $defaultNS['ns3'];
    $setparams['ns4'] = $defaultNS['ns4'];
    $setparams['ns5'] = $defaultNS['ns5'];

    return registrobr_SaveNameservers($setparams);
}
...
function registrobr_RegisterDomain($params){
    ...
    if (($isCPF == FALSE) and ($isCNPJ == FALSE) and !empty($params['additionalfields']['Register Number'])) {
        $RegistrantTaxID = $params['additionalfields']['Register Number'] ;
        if (isCpfValid($RegistrantTaxID)) {
            $isCPF = TRUE ;
        }
        if (isCnpjValid($RegistrantTaxID)) {
    ...
    # Use WHMCS nameservers if configured, otherwise fallback to auto.dns.br
    $Nameservers = array();
    for ($i = 1; $i <= 5; $i++) {
...
function registrobr_RenewDomain($params){
    ...
    // Atualiza nextduedate no WHMCS apos renovacao bem-sucedida
    if (!empty($values['expirydate']) && !empty($params['domainid'])) {
        localAPI('UpdateClientDomain', array(
            'domainid' => $params['domainid'],
            'nextduedate' => $values['expirydate'],
        ));
    }
...
# Function to accept domain transfer
# Registro.br does not support EPP transfer. The client changes the provider
# manually in the Registro.br panel. This function simply accepts the order
# so WHMCS marks the domain as "Pending Transfer". The TransferSync cron
# will detect when the provider change is complete and renew the domain.
...
function registrobr_TransferDomain($params) {
    ...
    logModuleCall(
        ...
        'TransferDomain',
        $params,
        'Transfer accepted - awaiting provider change at Registro.br panel for ' . $domain
    );
...
(
    ...
    // Atualiza nextduedate no WHMCS durante sync para manter consistencia
    if (!empty($nextduedate) && !empty($domainid)) {
        localAPI('UpdateClientDomain', array(
            'domainid' => $domainid,
...
# TransferSync - Called by WHMCS cron for domains in "Pending Transfer" status.
# Checks if the provider (clID) has been changed to ours at Registro.br,
# and if so, renews the domain. If renewal fails, opens an admin ticket.

function registrobr_TransferSync($params) {
    ...
    try {
        ...
        logModuleCall('registrobr', 'TransferSync', $params, $errorMsg);

        # Domain not found (2303) -> transfer failed
        if ($coderes == '2303') {
            return array(
                'failed' => true,
                'reason' => 'Domain not found at Registro.br: ' . $domain,
            );
        }

        # Any other error (connection, auth, etc) -> return empty, cron retries
        return array();
    }

    # Step 2 - Check if provider (clID) matches our username
    $clID = $objRegistroEPPDomain->get('clID');
    $ourProviderID = $moduleparams['Username'];

    if ($clID != $ourProviderID) {
        logModuleCall('registrobr', 'TransferSync', $params,
            'Provider not yet changed. clID=' . $clID . ' ours=' . $ourProviderID);
        # Provider not ours yet -> return empty, cron continues checking
        return array();
    }

    # Step 3 - Provider matches, attempt renewal (single attempt)
    $objRegistroEPPDomain->set('regperiod', 1);

    try {
        $objRegistroEPPDomain->renewDomain();
        $exDate = $objRegistroEPPDomain->get('exDate');
    }
    catch (Exception $e) {
        $renewError = $e->getMessage();
        logModuleCall('registrobr', 'TransferSync RenewFailed', $params, $renewError);

        # Renewal failed -> change status to Pending so cron stops running TransferSync
        localAPI('UpdateClientDomain', array(
            'domainid' => $domainid,
            'status' => 'Pending',
        ));

        # Open admin ticket with error details
        $ticketParams = array(
            'deptid' => $moduleparams['FinanceDept'],
            'subject' => 'Transfer renewal failed for ' . $domain,
            'message' => "The domain $domain (ID: $domainid) was transferred to our provider (clID: $clID), "
                       . "but the renewal attempt failed.\n\n"
                       . "Error: $renewError\n\n"
                       . "The domain status has been changed to Pending. Please resolve manually.",
            'priority' => 'High',
            'admin' => true,
        );

        if (!empty($params['userid'])) {
            $ticketParams['clientid'] = $params['userid'];
        } else {
            $ticketParams['email'] = 'noreply@registro.br';
            $ticketParams['name'] = 'Registro.br EPP';
        }

        $ticketResult = localAPI('OpenTicket', $ticketParams);
        if ($ticketResult['result'] != 'success') {
            logModuleCall('registrobr', 'TransferSync OpenTicket failed', $ticketParams, $ticketResult);
        }

        # Return empty so TransferSync doesn't override the status we just set
        return array();
    }

    # Renewal succeeded -> update nextduedate and mark as completed
    if (!empty($exDate) && !empty($domainid)) {
        localAPI('UpdateClientDomain', array(
            'domainid' => $domainid,
            'nextduedate' => $exDate,
        ));
    ...
    logModuleCall('registrobr', 'TransferSync Completed', $params,
        'Domain transferred and renewed. New exDate: ' . $exDate);
